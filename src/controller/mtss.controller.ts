...
export const assignTierAndIntervention = async (
  ...
  try {
    const { studentId, tier_level, notes, name, focus_area, frequency } =
      req.body;
...
export const getActiveTierCounts = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const tierCounts = await StudentTier.findAll({
      attributes: [
        "tierId",
        [Sequelize.fn("COUNT", Sequelize.col("studentId")), "studentCount"],
      ],
      where: {
        end_date: null, // Filter for active tiers
      },
      group: ["tierId", "Tier.name"],
      include: [
        {
          model: Tier,
          attributes: ["name"], // Include tier details
        },
      ],
      raw: true,
      nest: true,
    ...
    // The result from a grouped query with `raw: true` can be a bit nested.
    // We can re-map it to a cleaner format if needed.
    const formattedCounts = tierCounts.map((item: any) => ({
      tierId: item.tierId,
      tierLevel: item.Tier.tier_level,
      studentCount: parseInt(item.studentCount, 10),
    }));

    res.status(200).json(formattedCounts);
  } catch (error) {
    console.error("Error fetching active tier counts:", error);
    res.status(500).json({ error: "Failed to fetch active tier counts" });
  }
};

export const getTopTiers = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const { limit = 10 } = req.query; // Default to top 10 if not specified

    //get tiers 2 and 3 with x amount of students
    const tiersWithStudents = await Tier.findAll({
      where: {
        id: { [Op.in]: [2, 3] }, // Tiers 2 and 3
      },
      include: [
        {
          model: StudentTier,
          as: "StudentTiers",
          where: {
            end_date: null, // Active tiers only
          },
          include: [
            {
              model: Student,
    ...
    // Manually limit the number of students for each tier
    const result = tiersWithStudents.map((tier: any) => {
      const limitedStudents = tier.StudentTiers.slice(0, Number(limit)).map(
        (st: any) => st.Student
      );
      return {
        tierId: tier.id,
    ...
    console.error("Error fetching top interventions:", error);
    res.status(500).json({ error: "Failed to fetch top interventions" });
  }
};
