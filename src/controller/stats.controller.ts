...
export const getGradeCounts = async (req: Request, res: Response): Promise<void> => {
...
export const getEthnicityCounts = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (school) {
            // Allow multiple schools: school=1 or school=1,2,3
            const schoolIds = Array.isArray(school)
                ? (school as string[]).map((id) => parseInt(id, 10))
                : (school as string).split(",").map((id) => parseInt(id, 10))
            if (schoolIds.some(isNaN)) {
                res.status(400).json({ error: "Invalid school id(s)" })
                return
            }
            whereClause.school_id = schoolIds.length === 1 ? schoolIds[0] : { [Op.in]: schoolIds }
        }
        if (grade) {
        ...
        const ethnicityCounts = await Student.findAll({
            attributes: ["ethnicity", [sequelize.fn("COUNT", sequelize.col("ethnicity")), "count"]],
            where: whereClause,
            group: ["ethnicity"],
        })
...
export const getSchoolPerformance = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        console.error(error)
        res.status(500).json({ error: "Failed to retrieve school performance data" })
    }
}
